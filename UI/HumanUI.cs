		...
		private void RefreshDisplayData() {
			if (refeshOnce || (CitizenData.Instance.lastCitizenID != WorldInfoPanel.GetCurrentInstanceID().Citizen)) {
				if (isVisible) {
					...
					ushort homeBuilding = instance3.m_citizens.m_buffer[(int)((UIntPtr)CitizenData.Instance.lastCitizenID)].m_homeBuilding;
					BuildingManager instance2 = Singleton<BuildingManager>.instance;
					uint homeId = instance3.m_citizens.m_buffer[CitizenData.Instance.lastCitizenID].GetContainingUnit(CitizenData.Instance.lastCitizenID, instance2.m_buildings.m_buffer[homeBuilding].m_citizenUnits, CitizenUnit.Flags.Home);
					familyMoney.text = string.Format(Localization.Get("FAMILY_MONEY") + " [{0}]", CitizenUnitData.familyMoney[homeId]) + $"ID = ({homeId})";
					citizenMoney.text = string.Format(Localization.Get("CITIZEN_MONEY") + " [{0}]", CitizenData.Instance.citizenMoney[CitizenData.Instance.lastCitizenID]);
					familySalary.text = string.Format(Localization.Get("FAMILY_SALARY") + " [{0}]", CaculateFamilySalary(homeId).ToString());

					if ((instance3.m_citizens.m_buffer[CitizenData.Instance.lastCitizenID].m_flags & Citizen.Flags.NeedGoods) != 0) {
